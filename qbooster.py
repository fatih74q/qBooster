...
class SystemOptimizer:
    ...
    def get_power(self):
        try:
            r = subprocess.run(["powercfg", "/getactivescheme"], capture_output=True, text=True, creationflags=0x08000000)
...
class QBoosterApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        tr = ctk.CTkFrame(tc, fg_color="transparent"); tr.pack(fill="x", padx=15, pady=(5, 15))
        self.dns_btn = ctk.CTkButton(tr, text=self.t("dns_btn"), height=40,
                                     font=ctk.CTkFont(size=13, weight="bold"),
                                     fg_color=COLORS["accent_blue_light"],
                                     hover_color=COLORS["btn_secondary_hover"],
                                     command=self._dns_thread)
        self.dns_btn.pack(side="left", fill="x", expand=True, padx=(0, 5))
        self.junk_btn = ctk.CTkButton(tr, text=self.t("junk_btn"), height=40,
                                      font=ctk.CTkFont(size=13, weight="bold"),
